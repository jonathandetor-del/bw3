...
public class ShopManager extends ConfigManager {
    ...
    private static final HashMap<String, YamlConfiguration> groupShopOverrides = new HashMap<>();
    ...
    /**
     * Load group-specific shop override files (_groupname.yml).
     * These files override prices and currencies from the default shop.yml
     * for arenas belonging to the specified group.
     */
    private void loadGroupOverrides() {
        groupShopOverrides.clear();
        File dataFolder = BedWars.plugin.getDataFolder();
        File[] files = dataFolder.listFiles((dir, name) -> name.startsWith("shop_") && name.endsWith(".yml"));
        if (files == null) return;

        for (File file : files) {
            String groupName = file.getName().substring(5, file.getName().length() - 4);
            if (groupName.isEmpty()) continue;
            YamlConfiguration yml = YamlConfiguration.loadConfiguration(file);
            groupShopOverrides.put(groupName.toLowerCase(), yml);
            BedWars.plugin.getLogger().info("Loaded shop override for group: " + groupName);
        }
    }

    /**
     * Get the overridden price for a tier in a specific arena group.
     * Returns -1 if no override exists.
     *
     * @param group    the arena group name
     * @param tierPath the full config path of the tier
     * @return overridden price or -1 if not overridden
     */
    public static int getGroupPriceOverride(String group, String tierPath) {
        if (group == null || tierPath == null) return -1;
        YamlConfiguration yml = groupShopOverrides.get(group.toLowerCase());
        if (yml == null) return -1;
        String key = tierPath + ConfigPath.SHOP_CONTENT_TIER_SETTINGS_COST;
        if (yml.get(key) != null) {
    ...
    /**
     * Get the overridden currency for a tier in a specific arena group.
     * Returns null if no override exists.
     *
     * @param group    the arena group name
     * @param tierPath the full config path of the tier
     * @return overridden currency material or null if not overridden
     */
    public static Material getGroupCurrencyOverride(String group, String tierPath) {
        if (group == null || tierPath == null) return null;
        YamlConfiguration yml = groupShopOverrides.get(group.toLowerCase());
        if (yml == null) return null;
        String key = tierPath + ConfigPath.SHOP_CONTENT_TIER_SETTINGS_CURRENCY;
        String currency = yml.getString(key);
        if (currency == null || currency.isEmpty()) return null;
        return com.andrei1058.bedwars.shop.main.CategoryContent.getCurrency(currency.toLowerCase());
    }
