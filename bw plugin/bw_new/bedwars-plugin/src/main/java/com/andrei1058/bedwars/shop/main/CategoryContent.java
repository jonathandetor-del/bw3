...
public class CategoryContent implements ICategoryContent {
    ...
    public void execute(Player player, ShopCache shopCache, int slot) {
        ...
        //check money - resolve group-specific price/currency overrides
        IArena arena = Arena.getArenaByPlayer(player);
        String arenaGroup = arena != null ? arena.getGroup() : null;
        int tierPrice = ct.getPrice();
        Material tierCurrency = ct.getCurrency();
        if (arenaGroup != null && ct instanceof ContentTier) {
            String tierPath = ((ContentTier) ct).getTierPath();
            int overridePrice = ShopManager.getGroupPriceOverride(arenaGroup, tierPath);
            if (overridePrice >= 0) tierPrice = overridePrice;
            Material overrideCurrency = ShopManager.getGroupCurrencyOverride(arenaGroup, tierPath);
            if (overrideCurrency != null) tierCurrency = overrideCurrency;
        }

        int money = calculateMoney(player, tierCurrency);
        if (money < tierPrice) {
            player.sendMessage(getMsg(player, Messages.SHOP_INSUFFICIENT_MONEY).replace("{currency}", getMsg(player, getCurrencyMsgPath(tierCurrency, tierPrice))).
                    replace("{amount}", String.valueOf(tierPrice - money)));
            Sounds.playSound(ConfigPath.SOUNDS_INSUFF_MONEY, player);
            return;
        }
        ...
        ShopBuyEvent event;
        //call shop buy event
        Bukkit.getPluginManager().callEvent(event = new ShopBuyEvent(player, arena, this));
        ...
        //take money
        takeMoney(player, tierCurrency, tierPrice);
    ...
    public ItemStack getItemStack(Player player, ShopCache shopCache) {
        ...
        // Resolve group-specific price/currency overrides for display
        IArena arena = Arena.getArenaByPlayer(player);
        String arenaGroup = arena != null ? arena.getGroup() : null;
        int displayPrice = ct.getPrice();
        Material displayCurrency = ct.getCurrency();
        if (arenaGroup != null && ct instanceof ContentTier) {
            String tierPath = ((ContentTier) ct).getTierPath();
            int overridePrice = ShopManager.getGroupPriceOverride(arenaGroup, tierPath);
            if (overridePrice >= 0) displayPrice = overridePrice;
            Material overrideCurrency = ShopManager.getGroupCurrencyOverride(arenaGroup, tierPath);
            if (overrideCurrency != null) displayCurrency = overrideCurrency;
        }
        ...
        if (im != null) {
            ...
            boolean canAfford = calculateMoney(player, displayCurrency) >= displayPrice;
            ...
            String translatedCurrency = getMsg(player, getCurrencyMsgPath(displayCurrency, displayPrice));
            ChatColor cColor = getCurrencyColor(displayCurrency);
    ...
    public static String getCurrencyMsgPath(IContentTier contentTier) {
        return getCurrencyMsgPath(contentTier.getCurrency(), contentTier.getPrice());
    }

    /**
     * Get currency message path from material and price directly.
     * Used when group-specific overrides change the currency/price.
     */
    public static String getCurrencyMsgPath(Material currency, int price) {
        String c;

        if (currency.toString().toLowerCase().contains("iron")) {
            c = price == 1 ? Messages.MEANING_IRON_SINGULAR : Messages.MEANING_IRON_PLURAL;
        } else if (currency.toString().toLowerCase().contains("gold")) {
            c = price == 1 ? Messages.MEANING_GOLD_SINGULAR : Messages.MEANING_GOLD_PLURAL;
        } else if (currency.toString().toLowerCase().contains("emerald")) {
            c = price == 1 ? Messages.MEANING_EMERALD_SINGULAR : Messages.MEANING_EMERALD_PLURAL;
        } else if (currency.toString().toLowerCase().contains("diamond")) {
            c = price == 1 ? Messages.MEANING_DIAMOND_SINGULAR : Messages.MEANING_DIAMOND_PLURAL;
        } else {
            c = price == 1 ? Messages.MEANING_VAULT_SINGULAR : Messages.MEANING_VAULT_PLURAL;
        }
        return c;
    }
