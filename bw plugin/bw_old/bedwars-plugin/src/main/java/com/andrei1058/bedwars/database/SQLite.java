...
public class SQLite implements Database {
    ...
    public void init() {
        ...
        try {
            ...
            try (Statement st = connection.createStatement()) {
                st.executeUpdate("ALTER TABLE hotbar_preferences ADD COLUMN blocks2 INTEGER DEFAULT -1;");
            } catch (SQLException ignored) { /* column already exists */ }
            try (Statement st = connection.createStatement()) {
                st.executeUpdate("ALTER TABLE hotbar_preferences ADD COLUMN utility2 INTEGER DEFAULT -1;");
            } catch (SQLException ignored) { /* column already exists */ }
