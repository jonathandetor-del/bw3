...
public class SidebarService implements ISidebarService {
    ...
    /**
     * Refresh tab format for a specific player across all viewers in the same arena.
     * Used when team preference changes so tab colors update immediately.
     */
    public void refreshTabForPlayer(IArena arena, Player player) {
        this.sidebars.forEach((k, v) -> {
            if (null != v.getArena() && v.getArena().equals(arena)) {
                v.giveUpdateTabFormat(player, false);
            }
