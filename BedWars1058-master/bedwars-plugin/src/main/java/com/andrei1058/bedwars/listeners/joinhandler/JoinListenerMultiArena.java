...
public class JoinListenerMultiArena implements Listener {
    ...
    public void onJoin(PlayerJoinEvent e) {
        ...
        // Send items (delayed to ensure teleport completes — async on Paper forks)
        Bukkit.getScheduler().runTaskLater(plugin, () -> Arena.sendLobbyCommandItems(p), 5L);
    ...
    public void onWorldChange(PlayerChangedWorldEvent e) {
        ...
        if (BedWars.config.getLobbyWorldName().equalsIgnoreCase(p.getWorld().getName())) {
            Bukkit.getScheduler().runTaskLater(plugin, () -> Arena.sendLobbyCommandItems(p), 3L);
        }
    }
}
