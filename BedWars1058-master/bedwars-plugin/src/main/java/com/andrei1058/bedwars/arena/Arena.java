...
public class Arena implements IArena {
    ...
    public void init(World world) {
        ...
        // Sort teams by TeamColor enum ordinal: RED, BLUE, GREEN, YELLOW, AQUA, WHITE, PINK, GRAY
        teams.sort(java.util.Comparator.comparingInt(t -> t.getColor().ordinal()));
    ...
    public void removePlayer(@NotNull Player p, boolean disconnect) {
        ...
        TeamSelectorGUI.clearPreference(p.getUniqueId());
    ...
    public void changeStatus(GameState status) {
        ...
        if (status == GameState.starting) {
            ...
            // If only 1 player, use a longer countdown (90 seconds)
            if (players.size() == 1) {
                startingTask.setCountdown(90);
            }
        } else if (status == GameState.playing) {
    ...
    public static void sendLobbyCommandItems(Player p) {
        if (config.getYml().get(ConfigPath.GENERAL_CONFIGURATION_LOBBY_ITEMS_PATH) == null) {
            BedWars.plugin.getLogger().warning("[LobbyItems] lobby-items section is NULL in config! Items cannot be given.");
            return;
        }
        if (!BedWars.config.getLobbyWorldName().equalsIgnoreCase(p.getWorld().getName())) {
            BedWars.plugin.getLogger().warning("[LobbyItems] World mismatch: lobbyWorld='" + BedWars.config.getLobbyWorldName() + "' playerWorld='" + p.getWorld().getName() + "'");
            return;
        }
        p.getInventory().clear();
    ...
    public void sendPreGameCommandItems(Player p) {
        ...
        // Add team selector compass in slot 4
        p.getInventory().setItem(4, TeamSelectorGUI.createCompassItem());
    }
                    ...
                    if (null != topInChat) {
                        ...
                        for (Player receiver : receivers) {
                            ...
                            if (winner.getMembers().contains(receiver) || winner.wasMember(receiver.getUniqueId())) {
                                ...
                                receiver.playSound(receiver.getLocation(), "random.levelup", 1.0f, 1.0f);
                            } else {
