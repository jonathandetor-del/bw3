...
public class TeamAssigner implements ITeamAssigner {
    ...
    public void assignTeams(IArena arena) {
        ...
        // First pass: honor team selector preferences
        for (Player player : new ArrayList<>(arena.getPlayers())) {
            String pref = TeamSelectorGUI.getPreference(player.getUniqueId());
            if (pref == null) continue;
            for (ITeam team : arena.getTeams()) {
                if (team.getName().equals(pref) && team.getMembers().size() < arena.getMaxInTeam()) {
