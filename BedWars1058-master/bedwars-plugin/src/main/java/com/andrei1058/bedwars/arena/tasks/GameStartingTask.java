...
public class GameStartingTask implements Runnable, StartingTask {
    ...
    public void run() {
        ...
        if (getCountdown() % 10 == 0 || getCountdown() <= 5) {
            if (getCountdown() <= 5) {
                // Play ascending-pitch orb sound for 5-4-3-2-1 countdown
                // NOTE: note.pling is broken on PandaSpigot+ViaVersion (1.8 sound mapping EOL),
                // random.orb works reliably across all client versions.
                float pitch;
                switch (getCountdown()) {
                    case 5: pitch = 0.7f; break;
                    case 4: pitch = 0.8f; break;
                    case 3: pitch = 1.0f; break;
                    case 2: pitch = 1.2f; break;
                    case 1: pitch = 2.0f; break;
                    default: pitch = 1.0f;
                }
                for (Player p : getArena().getPlayers()) {
                    p.playSound(p.getLocation(), "random.orb", 1.0f, pitch);
                }
            } else {
