...
public class LegacyTeamAssigner {
    ...
    public static void assignTeams(IArena arena){
        //Honor team selector preferences first
        List<Player> skip = new ArrayList<>(), owners = new ArrayList<>();
        for (Player p : new ArrayList<>(arena.getPlayers())) {
            String pref = TeamSelectorGUI.getPreference(p.getUniqueId());
            if (pref == null) continue;
            for (ITeam t : arena.getTeams()) {
                if (t.getName().equals(pref) && t.getMembers().size() < arena.getMaxInTeam()) {
