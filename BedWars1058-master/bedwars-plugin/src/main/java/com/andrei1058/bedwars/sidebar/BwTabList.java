...
import com.andrei1058.bedwars.arena.team.TeamSelectorGUI;
...
public class BwTabList {
    ...
    public void giveUpdateTabFormat(@NotNull Player player, boolean skipStateCheck, @Nullable Boolean spectator) {
        ...
        // Reset any custom playerListName (e.g. set by Skript/TAB plugins in lobby)
        // so BedWars's scoreboard team prefix/suffix can control the tab display
        if (arena != null) {
        ...
        if (status != GameState.playing) {
            ...
            switch (status) {
                case waiting:
                    {
                        ITeam wTeam = arena.getTeam(player);
                        if (wTeam == null) {
                            wTeam = getPreferredTeam(arena, player);
                        }
                        HashMap<String, String> wReplacements = getTeamReplacements(wTeam);
                        prefix = getTabText(Messages.FORMATTING_SB_TAB_WAITING_PREFIX, player, wReplacements);
                        suffix = getTabText(Messages.FORMATTING_SB_TAB_WAITING_SUFFIX, player, wReplacements);
                    }
                    break;
                case starting:
                    {
                        ITeam sTeam = arena.getTeam(player);
                        if (sTeam == null) {
                            sTeam = getPreferredTeam(arena, player);
                        }
                        HashMap<String, String> sReplacements = getTeamReplacements(sTeam);
                        prefix = getTabText(Messages.FORMATTING_SB_TAB_STARTING_PREFIX, player, sReplacements);
                        suffix = getTabText(Messages.FORMATTING_SB_TAB_STARTING_SUFFIX, player, sReplacements);
                    }
                    break;
                case restarting:
    ...
    @Nullable
    private ITeam getPreferredTeam(IArena arena, Player player) {
        String prefName = TeamSelectorGUI.getPreference(player.getUniqueId());
        if (prefName == null) return null;
        for (ITeam t : arena.getTeams()) {
            if (t.getName().equals(prefName)) return t;
        }
        return null;
    }
    ...
    @NotNull HashMap<String, String> getTeamReplacements(@Nullable ITeam team) {
        ...
        replacements.put("{teamColor}", null == team ? "\u00a7f" : team.getColor().chat().toString());
